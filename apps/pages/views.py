...
def search_pages(request):
    ...
    object_list = Page.objects.filter(name__contains=search_text).order_by('-modified')
    
    return render_to_response('pages/ajax_result.html', {'object_list': object_list})

class PageIndexView(generic.CreateView):
     
    model = Page
    template_name = 'pages/index.html'
    form_class = PageIndexForm
      
      
    def get_context_data(self, **kwargs):
        context = super(PageIndexView, self).get_context_data(**kwargs)
        context['object_list'] = Page.objects.all().order_by('-modified')
